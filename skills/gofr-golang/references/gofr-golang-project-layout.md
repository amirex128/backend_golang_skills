...
## GoFr + Clean Architecture + CQRS + Repository Pattern

Use this project layout for GoFr services:

```text
cmd/api/main.go                         # calls gofr.New(), wires, starts
internal/domain/<bounded-context>/      # GoFr-free aggregates and invariants
internal/application/command/<feature>/ # commands and handlers
internal/application/query/<feature>/   # queries and read models
internal/application/ports/              # consumer-owned interfaces
internal/adapter/gofrhttp/               # GoFr routes, binders, presenters
internal/adapter/grpc/                   # optional GoFr gRPC adapters
internal/infrastructure/persistence/    # SQL/NoSQL implementations
internal/infrastructure/gofr/            # GoFr datasource/config adapters
internal/composition/                    # concrete dependency graph
api/                                     # OpenAPI/proto source contracts
migrations/                              # framework-supported migrations
```

Domain must not import GoFr, SQL, ORM, transport, or generated code. Application owns commands, queries, ports, authorization, and transaction boundaries. GoFr handlers only translate and call application handlers. A command loads and changes an aggregate through one aggregate repository; a query uses a purpose-built read port and returns a read model without hidden mutation. Keep GoFr datasource handles and SQL rows in infrastructure. Use transactional outbox when a command must atomically persist state and publish an event. Keep `cmd/api/main.go` thin and use manual constructor injection in the composition root.
