...
Keep GoFr request/response binding structs at the adapter boundary, application DTOs in application packages, domain types in domain packages, and datasource scan/ORM structs in infrastructure. Never let a GoFr-bound JSON shape become a domain aggregate by accident.
