...
## GoFr database adaptation

Use GoFr-managed configured datasources and the documented `*gofr.Context` datasource APIs. Keep SQL/NoSQL code in infrastructure repositories and query adapters; do not leak GoFr datasource types into domain or application. For writes, preserve aggregate invariants and transaction boundaries. For reads, use projection/query ports when the response is not an aggregate shape. Read the exact datasource page before using any driver-specific behavior.

## Required GoFr references

- `advanced-guide-dealing-with-sql.md` — https://gofr.dev/docs/advanced-guide/dealing-with-sql
- `datasources-getting-started.md` — https://gofr.dev/docs/datasources/getting-started
- `guides-connection-pooling.md` — https://gofr.dev/docs/guides/connection-pooling
- `advanced-guide-handling-data-migrations.md` — https://gofr.dev/docs/advanced-guide/handling-data-migrations
