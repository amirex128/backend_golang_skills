...
Use GoFr's configured datasource and `*gofr.Context` at the infrastructure adapter boundary. Read the GoFr SQL, datasource-getting-started, exact datasource, connection-pooling, and migration references before implementation. Do not put GoFr datasource handles, SQL rows, or ORM models in domain/application packages. Keep Repository Pattern and CQRS write/read boundaries from `gofr-golang-project-layout.md`.
