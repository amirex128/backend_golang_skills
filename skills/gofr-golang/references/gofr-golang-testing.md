...
Test the architecture in layers: pure domain/application unit tests; GoFr adapter tests for routes, binding, middleware, response/error mapping; datasource integration tests with isolated databases; and deployment/lifecycle tests for health, startup, shutdown, observability, cron, and Pub/Sub when relevant. Read `gofr-references-testing.md` and the feature-specific GoFr reference.
