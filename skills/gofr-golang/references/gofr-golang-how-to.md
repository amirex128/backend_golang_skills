...
Implement practical recipes through GoFr, beginning with `gofr.New()`, documented handler registration, GoFr configuration, configured datasources, and built-in observability. Read the exact GoFr feature reference before writing code; do not substitute generic net/http examples when GoFr provides a supported mechanism.
