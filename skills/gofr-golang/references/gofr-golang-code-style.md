...
Keep GoFr handlers thin and idiomatic: use the documented `func(c *gofr.Context) (any, error)` shape, return values/errors to GoFr, use `c.Bind` with checked errors, and do not duplicate GoFr response envelopes, logging, metrics, tracing, or health behavior. Apply the general Go style below inside domain/application/infrastructure packages.
